...
describe('GET /students/:id/action-center', () => {
    it('TEST 1 — returns full action center shape for stu_001', async () => {
        const res = await (0, supertest_1.default)(app_1.default).get('/students/stu_001/action-center');
    ...
    it('TEST 4 — returns 404 with requestId for unknown student', async () => {
        const res = await (0, supertest_1.default)(app_1.default).get('/students/stu_999/action-center');
...
describe('GET /counselor/:id/triage', () => {
    it('TEST 3 — students are ranked by urgency score descending', async () => {
        const res = await (0, supertest_1.default)(app_1.default).get('/counselor/csl_001/triage');
        expect(res.status).toBe(200);
        ...
        const scores = res.body.rankedStudents.map((entry) => entry.urgency.score);
        for (let i = 0; i < scores.length - 1; i++) {
            expect(scores[i]).toBeGreaterThanOrEqual(scores[i + 1]);
        }
    });
    it('returns 404 for unknown counselor', async () => {
        const res = await (0, supertest_1.default)(app_1.default).get('/counselor/csl_999/triage');
        expect(res.status).toBe(404);
        expect(res.body).toHaveProperty('error');
        expect(res.body).toHaveProperty('requestId');
    });
...
//# sourceMappingURL=actionCenter.test.js.map